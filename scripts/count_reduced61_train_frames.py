"""
Count per-class frame frequencies for reduced_61 (method B).

Same segmentation as ChordDataset (fixed segment_frames chunks over each song),
same full-vocab -> reduction_61 mapping as dataset.py. Does NOT apply padding or
...
Run from project root:
  python scripts/count_reduced61_train_frames.py --config config.yaml
  python scripts/count_reduced61_train_frames.py --config config.yaml --out-json data/reduced61_train_frame_counts.json
"""
...
def map_full_to_reduced61(labels_1d: np.ndarray, reduction: np.ndarray) -> np.ndarray:
    """Same logic as ChordDataset.__getitem__ (reduction_61 branch only)."""
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Count reduced_61 label frames per class (segment slicing = ChordDataset; no pad, no aug)."
    )
    ...
    with open(vocab_path, encoding="utf-8") as f:
        vocab = json.load(f)
    reduction_list = vocab.get("reduction_61")
    if not reduction_list:
        raise SystemExit("chord_vocabulary.json missing reduction_61 (need reduced_61 mapping).")
    num_reduced = int(vocab.get("num_classes_reduced_61") or 61)
    reduction = np.asarray(reduction_list, dtype=np.int64)
    nc_full = vocab.get("num_classes")
    if nc_full is not None and len(reduction) != int(nc_full):
        raise SystemExit(
            f"reduction_61 length {len(reduction)} != num_classes {nc_full} in vocabulary."
        )
    ...
    counts = np.zeros(num_reduced, dtype=np.int64)
    ...
    for index, start, end in segments:
        ...
        reduced = map_full_to_reduced61(sl, reduction)
        valid = (reduced != IGNORE_INDEX) & (reduced >= 0) & (reduced < num_reduced)
        ignored_after_reduce += int((~valid).sum())
        if valid.any():
            counts += np.bincount(reduced[valid], minlength=num_reduced)
    ...
    meta = {
        ...
        "num_classes_reduced_61": num_reduced,
    }

    print(json.dumps(meta, ensure_ascii=False, indent=2))
    print("class_id\tcount\tfraction")
    for c in range(num_reduced):
